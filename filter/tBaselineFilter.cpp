...
tBaselineFilter::tBaselineFilter(tWindow* window) :
    tFilter(window)
{
    setName(QObject::tr("Baseline"));
    if(!window) return;

    m_output = tMatrixData::Create();
    m_output->setName(QObject::tr("Baselined"));
...
tBaselineFilter::~tBaselineFilter()
{
    doCallbacks(sigDecaying);
}


void tBaselineFilter::setInputWindow(tWindow* window)
{
    m_input.setList(&window->getData());
}


void tBaselineFilter::run_impl(bool notify)
{
    if(!m_input) return;
