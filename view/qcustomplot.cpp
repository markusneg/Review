...
void QCPScatterStyle::setIntraPlotColors(const QCPColorGradient &intraPlotColors)
{
    mIntraPlotColors = intraPlotColors;
    //mUseIntraPlotColors = true;
}

void QCPScatterStyle::activateIntraPlotColoring(bool activate)
{
    mUseIntraPlotColors = activate;
}
...
/*!
  ...
  \see applyTo
*/
void QCPScatterStyle::drawShape(QCPPainter *painter, QPointF pos, const QCPData* data) const
{
  drawShape(painter, pos.x(), pos.y(), data);
}

/*! \overload
  Draws the scatter shape with \a painter at position \a x and \a y. \a data is passed for intra plot appearance
*/
void QCPScatterStyle::drawShape(QCPPainter *painter, double x, double y, const QCPData* data) const
{
  double w = mSize/2.0;

  if(mUseIntraPlotColors)
  {
      QPen pen = painter->pen();
      pen.setColor(mIntraPlotColors.color(data->color, mColorRange));
      painter->setPen(pen);
  }
...
/*!
  Let ScatterStyle gather information from data to enable correct intra plot appearance (coloring etc)
*/
void QCPScatterStyle::acquireDataRanges(const QCPDataMap* dataMap) const
{
    if(!dataMap || dataMap->empty()) return;

    if(mUseIntraPlotColors)
    {
        double min = std::numeric_limits<double>::max(), max = std::numeric_limits<double>::min();
        for(QCPDataMap::const_iterator it = dataMap->begin(); it != dataMap->end(); ++it)
        {
            double color = it.value().color;
            if(color < min) min = color;
            if(color > max) max = color;
        }

        mColorRange.lower = min; mColorRange.upper = max;
    }
}
...
/*! \internal
  ...
  performance.
*/
QRgb QCPColorGradient::color(double position, const QCPRange &range, bool logarithmic) const
{
...
*/
void QCPColorGradient::updateColorBuffer() const
{
...
/*! \class QCPData
  ...
  \li \a keyErrorMinus: negative error in the key dimension (for error bars) OR point / segment color
...
void QCPGraph::drawScatterPlot(QCPPainter *painter, QVector<QCPData> *scatterData) const
{
  ...
  mScatterStyle.acquireDataRanges(mData);
  ...
  if (keyAxis->orientation() == Qt::Vertical)
  {
    for (int i=0; i<scatterData->size(); ++i)
      if (!qIsNaN(scatterData->at(i).value))
        mScatterStyle.drawShape(painter, valueAxis->coordToPixel(scatterData->at(i).value), keyAxis->coordToPixel(scatterData->at(i).key), &scatterData->at(i));
  } else
  {
    for (int i=0; i<scatterData->size(); ++i)
      if (!qIsNaN(scatterData->at(i).value))
        mScatterStyle.drawShape(painter, keyAxis->coordToPixel(scatterData->at(i).key), valueAxis->coordToPixel(scatterData->at(i).value), &scatterData->at(i));
  }
}
